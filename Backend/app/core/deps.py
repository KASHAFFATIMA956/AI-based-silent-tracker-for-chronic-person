...
- is_authorized_to_send_message: RBAC for POST /patients/{patient_id}/messages
  (added 2026-09-07) — returns the MessageSenderRole to record if
  current_user may send a message for this patient, else None. Reads (GET)
  reuse get_authorized_patient like every other patient-scoped read; this
  is narrower and ONLY for the write path, since MessageSenderRole has no
  `admin` value — an admin can read any thread but has no sensible role to
  be recorded under if they sent one. See context/decisions-log.md.
"""
...
from app.models.enums import MessageSenderRole, UserRole
...
def is_authorized_to_send_message(
    current_user: User, patient: Patient
) -> MessageSenderRole | None:
    """
    RBAC for POST /patients/{patient_id}/messages: only the patient
    themselves, their attendant, or their assigned doctor may send a
    message for this patient — admin is deliberately excluded here (unlike
    every other is_authorized_for_X check above, which treats admin as
    always-allowed) because MessageSenderRole has no `admin` value to
    record the message under. Admin can still read the thread via the
    ordinary get_authorized_patient dependency on the GET route — this
    function is ONLY consulted on the write path. Returns the sender_role
    to persist, or None if current_user may not send. See
    context/decisions-log.md.
    """
    if current_user.id == patient.user_id:
        return MessageSenderRole.patient
    if current_user.id == patient.attendant_user_id:
        return MessageSenderRole.attendant
    if current_user.id == patient.assigned_doctor_id:
