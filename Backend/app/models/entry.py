...
context/decisions-log.md) — optional home-monitoring readings, following
the exact same precedent as weight_value: nullable, NOT gated to
entry_type (a patient with a home BP cuff or glucometer may log a reading
alongside either a voice or a quick entry), no default-value assumptions.
**Persisted only as of this pass — deliberately NOT wired into
app/services/rules.py's scoring/red-flag logic yet.** That needs its own
follow-up pass with real clinical threshold review (what counts as a
hypertensive-crisis or hypoglycemia red flag varies by diagnosis and
patient baseline, unlike weight's single doc-sourced heart-failure
threshold) — see context/decisions-log.md and context/progress.md's open
items. These values are visible to doctors (timeline/patient-detail) but
currently have zero effect on risk_level.
"""
...
class Entry(Base):
    ...
    # mmHg. Optional home-monitoring readings, not gated to entry_type —
    # see module docstring. NOT yet wired into app/services/rules.py.
    # Added 2026-09-07.
    ...
    # mg/dL. Same optional/not-gated/not-yet-scored contract as the BP
    # fields above. Added 2026-09-07.
    blood_sugar_mg_dl: Mapped[int | None] = mapped_column(Integer, nullable=True)
