  ...
  async function handleSubmit() {
    ...
    try {
      const entry = await submitEntry({
        ...
        systolicBp: systolic === '' ? null : Number(systolic),
        diastolicBp: diastolic === '' ? null : Number(diastolic),
        bloodSugarMgDl: bloodSugar === '' ? null : Number(bloodSugar),
  ...
  return (
    <div style={{ padding: '32px 40px', maxWidth: 820 }}>
      ...
      <div className="rn-card" style={{ padding: 26, display: 'flex', flexDirection: 'column', gap: 22 }}>
        ...
        {/* Added 2026-09-07 — same "clearly optional" treatment as the
            weight field above: muted helper copy, no red-asterisk
            required marker, placeholder text (not a filled value) so an
            empty field never looks incomplete. Persisted only — NOT yet
            part of risk scoring, see context/decisions-log.md. */}
        <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
          <span style={{ fontSize: 14 }}>Blood pressure (optional)</span>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, maxWidth: 320 }}>
            ...
            />
            <span className="rn-muted" style={{ fontSize: 12 }}>mmHg</span>
          </div>
        </div>
        ...
        <label style={{ display: 'flex', flexDirection: 'column', gap: 6, maxWidth: 260 }}>
          <span style={{ fontSize: 14 }}>Blood sugar (optional, mg/dL)</span>
          <input
            className="rn-input"
            type="number"
            placeholder="Only if you've checked it today"
            value={bloodSugar}
