...
The direct-messaging pass (2026-09-07, final session, see [[progress]])
added exactly one new table, `messages` (below) — a genuinely new schema
change, via `alembic/versions/97a9fb958383_add_messages_table.py` — and did
not touch or restructure any existing table.

Verified end-to-end against the project's own dev Postgres: `alembic upgrade
head` applied cleanly (including this session's new `97a9fb958383` migration),
`seed.py` populated all tables with no FK errors, and re-running `seed.py` is
idempotent (clears + re-inserts to identical row counts; `messages` has no
seed rows and is unaffected either way).
...
| `message_sender_role` | `MessageSenderRole` | patient, attendant, doctor — added 2026-09-07, see `messages` below |
...
| systolic_bp | integer, nullable | mmHg; NOT gated to entry_type; added 2026-09-07; **not yet read by the rule engine** |
| diastolic_bp | integer, nullable | mmHg; NOT gated to entry_type; added 2026-09-07; **not yet read by the rule engine** |
| blood_sugar_mg_dl | integer, nullable | mg/dL; NOT gated to entry_type; added 2026-09-07; **not yet read by the rule engine** |
...
**`systolic_bp`/`diastolic_bp`/`blood_sugar_mg_dl` added 2026-09-07** — optional
home-monitoring readings (blood pressure, blood sugar), following the exact same
precedent as `weight_value`: nullable, not gated to `entry_type`, no default-value
assumptions. Migration: `alembic/versions/95fc7e1824e7_add_entries_bp_and_blood_sugar.py`.
Bounds enforced at the API boundary (`app/schemas/entry.py`): systolic 40-300, diastolic
20-200, blood sugar 20-800 mg/dL — generous sanity checks, not clinical ranges, same
spirit as weight's 0-300kg bound. **Unlike `weight_value`, these three are deliberately
NOT wired into `app/services/rules.py`'s scoring/red-flag logic** — they are persisted
and returned on `EntryOut` (visible to doctors on the patient-detail view and to
patients on their own Timeline) but currently have zero effect on `risk_level`/alerts.
Wiring them into scoring is an explicit, separate future task requiring real clinical
threshold review (what counts as a hypertensive-crisis or hypo/hyperglycemia red flag
varies by diagnosis and patient baseline, unlike weight's single doc-sourced
heart-failure threshold) — see [[decisions-log]] and [[progress]]'s open items.
...
### messages
Added 2026-09-07 (direct-messaging pass) — one row per chat message, patient/attendant
<-> assigned-doctor. No existing table was touched or restructured to add this.
| Column | Type | Notes |
|---|---|---|
| id | PK int | |
| patient_id | FK patients | the thread this message belongs to |
| sender_user_id | FK users | who actually sent it |
| sender_role | message_sender_role | patient / attendant / doctor — see enum table above; deliberately no `admin` value |
| content | text | |
| created_at | timestamp | server default now() |
| read_at | timestamp, nullable | exists per spec; **nothing sets it this pass** — no mark-as-read endpoint, no read-receipts UI (explicitly out of scope) |

New Postgres enum `message_sender_role` (patient, attendant, doctor) — narrower than
`user_role`: an admin can read any thread (same RBAC as every other patient-scoped
read) but has no sensible role to be recorded under if they sent a message, so admin is
blocked on the write path entirely — see [[decisions-log]] and [[api-contracts]].
Migration: `alembic/versions/97a9fb958383_add_messages_table.py`, one clean
`create_table`, no data migration, matches the exact shape of every prior
additive-table/column migration in this project (autogenerate-style, hand-verified).
