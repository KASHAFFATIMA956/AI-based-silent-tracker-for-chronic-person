...
- `entry_type: "voice"` requires non-empty `raw_transcript`; `entry_type: "quick"`
  ...
  `systolic_bp`/`diastolic_bp`/`blood_sugar_mg_dl` do NOT count toward this check (see
  below).
...
- **`systolic_bp` (40-300), `diastolic_bp` (20-200), `blood_sugar_mg_dl` (20-800) —
  added 2026-09-07.** All three optional, nullable, not gated to `entry_type` (same
  contract as `weight_value`) — bounds are generous sanity checks, not clinical
  ranges. Deliberately excluded from the "quick entry needs at least one value" check
  (unlike `weight_value`, which does count) since they were never part of the original
  quick check-in spec — a request with only these three fields and every other quick
  field blank still gets a 422. **Persisted and returned only — NOT read by
  `app/services/rules.py`.** A reading here has zero effect on `risk_level`/alerts;
  wiring it in is an explicit future task needing clinical threshold review, see
  [[decisions-log]] and [[schema]].
...
```json
{
  ...
  "systolic_bp": null, "diastolic_bp": null, "blood_sugar_mg_dl": null,
  "medicine_status": "taken",
...
## Messages (direct messaging, added 2026-09-07)

Direct messaging between a patient (or their attendant) and their assigned doctor.
No websocket infrastructure exists in this backend (REST-only) and none was added —
the frontend polls `GET /patients/{patient_id}/messages` on an interval (12s on both
platforms) while its message screen is open, stopping when the screen is left. Real-time
push (websockets/SSE) is a possible future upgrade, not built this pass.

### `POST /patients/{patient_id}/messages`
**Auth**: required. RBAC is narrower than every other patient-scoped route above:
only the patient themselves, their attendant, or their assigned doctor may send a
message — **admin is blocked here**, unlike everywhere else in this app where admin is
always allowed, because there is no `admin` value in `message_sender_role` to record
the message under (`app.core.deps.is_authorized_to_send_message`). The patient is
still resolved via the ordinary `get_authorized_patient` dependency first (so a 404 on
a nonexistent patient, or a 403 for a completely unrelated user, look identical to
every other patient-scoped route) — the narrower check runs after that, only for
deciding whether *this* user may write.

Request body (`MessageCreate`):
```json
{ "content": "Hi doctor, I have been feeling more tired than usual." }
```
`content`: 1-4000 characters (a generous sanity bound, not a real product limit).
...
```json
{
  ...
  "sender_user_id": 48,
  "sender_role": "patient",
  "sender_name": "Zubaida Bibi",
  "content": "Hi doctor, I have been feeling more tired than usual.",
  "created_at": "2026-09-07T05:49:35.324528",
  "read_at": null
}
```
`sender_name` is computed via a lookup (same "not a plain column" pattern as
`DoctorNoteOut.doctor_name`). `read_at` is always `null` — no endpoint sets it this
pass (no mark-as-read call, no read-receipts UI — explicitly out of scope); it's
exposed only because the task's own schema spec calls for the column.

Response `403` — role/relationship isn't patient/attendant/assigned-doctor for this
patient (includes admin): `{"detail": "Only the patient, their attendant, or their
assigned doctor may send messages"}`. A completely unrelated user (or a doctor who
isn't this patient's assigned doctor) gets the ordinary
`{"detail": "Not authorized to access this patient"}` instead, from
`get_authorized_patient` itself, before this narrower check even runs.
Response `404` — no such patient: `{"detail": "Patient not found"}`.
Response `422` — empty or over-length `content`.

### `GET /patients/{patient_id}/messages`
**Auth**: required. Same RBAC as `GET /patients/{patient_id}`/the timeline endpoint
(`get_authorized_patient` — admin/assigned-doctor/self-patient/linked-attendant). Unlike
the write path above, admin CAN read any thread here — the narrower check only applies
to sending.

Returns the full thread, **oldest-first** — the one deliberate ordering exception in
this app (every other list endpoint — timeline, notes, alerts — is newest-first), since
a chat thread reads naturally top-to-bottom chronologically.

Response `200` (`list[MessageOut]`, same shape as the `POST` response):
```json
[
  { "id": 1, "sender_role": "patient", "sender_name": "Zubaida Bibi", "content": "Hi doctor, I have been feeling more tired than usual.", "...": "..." },
  { "id": 2, "sender_role": "doctor", "sender_name": "Dr. Ayesha Farooq", "content": "Thanks for letting me know, please monitor your weight daily.", "...": "..." },
  { "id": 3, "sender_role": "attendant", "sender_name": "Imran Zubair", "content": "This is Imran, her attendant. Noted, thank you.", "...": "..." }
]
```

Response `403`/`404` — same as `GET /patients/{patient_id}`. **A patient requesting
another patient's thread gets a real 403**, not someone else's messages — verified
directly (see Verified section below).
...
## Verified (2026-09-07, direct-messaging pass — final session)
Manually exercised `POST`/`GET /patients/{patient_id}/messages` against a freshly-
migrated dev Postgres (migration `97a9fb958383` applied cleanly on top of the existing
schema) + a running FastAPI instance, real HTTP requests, 12 checks, all passed:
- Zubaida Bibi (patient, self) posts a message — `201`, correct `sender_role: "patient"`.
- Dr. Ayesha Farooq (her assigned doctor) posts a reply — `201`, `sender_role: "doctor"`.
- Imran Zubair (her attendant) posts — `201`, `sender_role: "attendant"`.
- Dr. Hamza Iqbal (a doctor, but NOT assigned to this patient) posts — `403`
  (`"Not authorized to access this patient"`, from `get_authorized_patient` itself).
- Sadia Kamran (admin) posts — `403` (`"Only the patient, their attendant, or their
  assigned doctor may send messages"`) — **confirms admin is correctly blocked from
  sending**, the one deliberate asymmetry from every other patient-scoped route.
- Ghulam Rasool (a different, unrelated patient) posts — `403`.
- **Ghulam Rasool (a different patient) attempts to GET this patient's thread — `403`,
  not someone else's messages** — the exact cross-patient-read check the task asked to
  confirm.
- Zubaida reads her own thread — `200`, all 3 messages returned, correctly
  **oldest-first** (patient -> doctor -> attendant, matching send order).
- Admin reads the same thread — `200`, all 3 messages visible (read access is NOT
  narrowed the way write access is).
- Unauthenticated request — `401`.
- Nonexistent patient id — `404`.
- Empty `content` — `422` (`"String should have at least 1 character"`).
`python seed.py` re-ran cleanly against the new schema with zero errors (a brand-new
table needs no seed-script changes). `flutter analyze` (mobile) came back with only the
same pre-existing style-level `info` lints already present elsewhere in this codebase —
zero new errors/warnings from this pass's code. `npm run build` (web) completed clean;
`npm run lint` (oxlint) showed only the same pre-existing `only-export-components`
warnings already present elsewhere, plus one new `set-state-in-effect` info-level
warning on `MessagesScreen.jsx`'s data-fetch effect (a standard, harmless "fetch on
mount" pattern oxlint flags because it can trace directly into the same-file
`load()` closure — not a bug, same tolerated-warning category as the pre-existing ones).
...
## Verified (2026-09-07, blood pressure / blood sugar fields pass)
Manually exercised `POST /entries` against a freshly-migrated dev Postgres (migration
`95fc7e1824e7` applied cleanly on top of the existing schema) + a running FastAPI
instance: an entry submitted WITH `systolic_bp`/`diastolic_bp`/`blood_sugar_mg_dl` set
persisted all three correctly (confirmed via the response body AND a direct Postgres
`SELECT`); an entry submitted WITHOUT them (all three omitted) saved with all three
`null`, no error, no default-value substitution. Both entries produced byte-identical
`risk_result` shapes (`Green`, identical `reasoning`) — confirms the new fields have
literally zero effect on the rule engine, as designed. A separate chest-pain entry on
the same patient still correctly hard-flagged `Red`, confirming the pre-existing
scoring logic is unmodified. Out-of-range `systolic_bp` (9999) correctly `422`s. `python
seed.py` re-ran cleanly against the new schema with zero errors (nullable columns need
no seed-side changes). `flutter analyze` (mobile) and `npm run build` (web) both clean
